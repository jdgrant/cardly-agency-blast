...
const Step7SelectPackage = () => {
  const { state, updateState, nextStep, prevStep, pricingTiers } = useWizard();

  // Auto-select package based on client count
  useEffect(() => {
    if (state.clientList.length > 0 && !state.selectedTier) {
      const clientCount = state.clientList.length;
      
      // Find the appropriate tier based on client count
      let selectedTier = pricingTiers[0]; // Default to smallest
      
      for (const tier of pricingTiers) {
        if (clientCount <= tier.quantity) {
          selectedTier = tier;
          break;
        }
      }
      
      // If client count exceeds largest tier, select the largest
      if (clientCount > pricingTiers[pricingTiers.length - 1].quantity) {
        selectedTier = pricingTiers[pricingTiers.length - 1];
      }
      
      updateState({ selectedTier });
    }
  }, [state.clientList.length, state.selectedTier, pricingTiers, updateState]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
        {pricingTiers.map((tier) => {
          ...
          const isRecommended = clientCount > 0 && clientCount <= tier.quantity && 
            (pricingTiers.find(t => clientCount <= t.quantity)?.name === tier.name);
              ...
              <CardHeader className="text-center pb-4">
                ...
                {clientCount > 0 && (
                  <div className="text-sm text-gray-600">
                    {clientCount <= tier.quantity ? 
                      `Covers all ${clientCount} clients` :
                      `${tier.quantity - clientCount} cards remaining`
                    }
                  </div>
                )}
...
export default Step7SelectPackage;